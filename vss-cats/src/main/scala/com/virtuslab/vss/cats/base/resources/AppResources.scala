...
object AppResources {
  ...
  /**
    ...
    *
    * This needs to be created in a separate step, because it will be instantiated with a different monad. That is
    * because the entry point is used to initialize the tracing monad and the other resources have to be in the context
    * of the tracing monad, since they will be passed to the service layer.
    *
    * TLDR: if the entry point is created in monad `F[_]`, other resources will have to be created in monad `Span[F] =>
    * F[_]`.
    */
